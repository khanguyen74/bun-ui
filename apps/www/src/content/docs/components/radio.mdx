...
- The `RadioGroup` component ensures that only one option can be selected at a time.
- The `label` prop in `RadioGroupItem` links an accessible label to the radio button.
