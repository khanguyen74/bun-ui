---
title: Select
description: A dropdown component for selecting a single option from a list.
links:
  source: components/select
---

<ComponentPreview name="select-basic" />

## Usage

Import the `Select` component and its related subcomponents from `@bun-ui/react` and use them in your project:

```tsx
import { Select, SelectItem } from "@bun-ui/react"
```

```tsx
<Select placeholder="Select an option">
  <SelectItem value="option1">Option 1</SelectItem>
  <SelectItem value="option2">Option 2</SelectItem>
  <SelectItem value="option3">Option 3</SelectItem>
</Select>
```

## Groups and Labels

The `Select` component supports grouping options with labels for better organization.

<ComponentPreview name="select-grouped" />

## Controlled

<ComponentPreview name="select-controlled" />

## Disabled

The `Select` comonent can be disable as a whole or individual items can be disabled.

<ComponentPreview name="select-disabled" />

## Customization

The `Select` component and its subcomponents can be customized using the `className` prop or by extending their styles with your design system.

<ComponentPreview name="select-customized" />

## Notes

- The `Select` component uses `@radix-ui/react-select` for its base functionality.
- The `SelectItem` component represents individual options in the dropdown.
- The `SelectGroup` and `SelectLabel` components allow for grouping and labeling options.
- The `menuPosition` prop can be used to control the dropdown's position (`"popper"` or `"item-aligned"`).

Refer to the source code for more advanced usage and customization options.
