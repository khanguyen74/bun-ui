...
```jsx
import { FileUpload, FileUploadDropZone, FileUploadTrigger } from "@bun-ui/react"
```
...
Use a custom button as the file selection trigger.
...
You can preview the selected files before uploading them by using `FileUploadPreview`
