...
The `Button` component is built with accessibility in mind. Ensure that buttons
have accessible labels and are keyboard-navigable. Use the `aria-*` attributes
where necessary to provide additional context.
...
The `Button` component can be customized using the `className` prop or by
...
- The `Button` component uses `class-variance-authority` for managing variants
  and sizes.
- The `asChild` prop allows you to render the button as a custom component, such
  as a `Link` or `div`.
